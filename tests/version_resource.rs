...
//! the finished, linked artefact can answer whether it landed. `CARGO_BIN_EXE_githoot` is
...
//!
//! `src/version.rs` prefers `GITHOOT_VERSION` over `Cargo.toml`, and `build.rs` has to resolve the
...
const EXE: &str = env!("CARGO_BIN_EXE_githoot");
...
fn version_resource_identifies_the_publisher() {
    ...
    assert_eq!(string_value(&block, "ProductName"), "githoot");
    assert_eq!(
        string_value(&block, "OriginalFilename"),
        "githoot.exe"
    );
    assert!(
        string_value(&block, "FileDescription").contains("githoot"),
