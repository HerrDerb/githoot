...
//!
//! The shipped `githoot.exe` was flagged by Windows Defender as
...
//!
//! `src/version.rs` prefers `GITHOOT_VERSION` (the git tag, injected by CI) over `Cargo.toml`, and notes
...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=GITHOOT_VERSION");
    println!("cargo:rerun-if-changed=assets/tray.png");
    println!("cargo:rerun-if-changed=windows/githoot.manifest");
...
fn windows_resource() {
    ...
    res.set("ProductName", "githoot");
    res.set(
        "FileDescription",
        "githoot - GitHub pull request tray icon",
    );
    ...
    res.set("OriginalFilename", "githoot.exe");
    ...
    res.set_manifest_file("windows/githoot.manifest");
...
fn resolve_version() -> String {
    match std::env::var("GITHOOT_VERSION") {
...
fn generate_icon() -> Result<String, Box<dyn std::error::Error>> {
    ...
    let ico_path = std::path::Path::new(&out_dir).join("githoot.ico");
