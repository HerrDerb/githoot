...
mod render_dump {
    ...
    /// Dumps every built variant to `$GITHOOT_ICON_DUMP` for visual inspection.
    ///
    ...
    /// `GITHOOT_ICON_DUMP=/tmp/icons cargo test -- --ignored dump_all_variants --nocapture`.
    #[test]
    #[ignore = "writes PNGs for a human to look at"]
    fn dump_all_variants() {
        let Ok(dir) = std::env::var("GITHOOT_ICON_DUMP") else { return };
